...
Media messages appear as placeholders like "[voice message]", "[image]", or "[document]" since the actual content isn't available to you. A trailing unanswered media message from the Lead — especially a voice message — generally DOES require a personal response from Didi. Exception: "[template message]" marks an automated broadcast/marketing blast (insurer newsletters, promos) and does NOT require a response.
...
function buildTranscript(
  chatId: string,
  history: { type: "incoming" | "outgoing"; textMessage?: string; timestamp: number; typeMessage?: string }[],
): string {
  const sorted = [...history].sort((a, b) => a.timestamp - b.timestamp);
  const lines = sorted
    .filter((m) => m.typeMessage !== "reactionMessage")
    .map((m) => {
      const text = m.textMessage && m.textMessage.trim() ? m.textMessage : MEDIA_PLACEHOLDERS[m.typeMessage ?? ""] ?? "[message]";
      return `[${israelLineLabel(new Date(m.timestamp * 1000))}] ${m.type === "outgoing" ? "Didi" : "Lead"}: ${text}`;
    });
